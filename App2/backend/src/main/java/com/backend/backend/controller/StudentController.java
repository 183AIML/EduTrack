...
public class StudentController {
    ...
    // This assumes you pass the user's email as a header (replace with JWT logic as needed)
    @GetMapping("/me")
    public ResponseEntity<?> getMyDetails(@RequestHeader("X-User-Email") String email) {
        return userRepository.findByEmail(email)
            .map(user -> {
                ...
                System.out.println("[DEBUG] Department for student " + student.getRegNumber() + ": " + student.getDepartment());
